...
under test:
  ...
  3. The wallet pubkey is read from the bounty_predictions table
     (not from subscriptions) — this is the fix for the wallet-lookup bug.
  4. A simulated channel payment is recorded in bounty_payments.
...
class FakePaymentAdapter:
    ...
    Mirrors what _bountyPayPredictor does: opens a channel if needed,
...
def score_and_pay(
    ...
    """Host-side pipeline that mirrors _bountyScoreAndPay +
    _bountyPayPredictor without async/network concerns.
    ...
    predictions = db.get_bounty_predictions(
        stream_name, stream_provider_pubkey, seq_num)
    if not predictions:
        return

    payouts = compute_payouts(bounty, predictions, observed)
...
def _add_bounty(db, pay=1000, paid_predictors=3):
    db.add_bounty(
        stream_name=STREAM,
...
class TestWalletPubkeyPropagation:
    ...
    def test_wallet_pubkey_stored_with_prediction(self, db):
        ...
        rows = db.get_bounty_predictions(STREAM, PROVIDER_PK, 1)
        assert len(rows) == 1
    ...
    def test_predictor_with_no_wallet_pubkey_is_skipped(self, db, adapter):
        ...
        assert db.get_bounty_payments(STREAM, PROVIDER_PK) == []
...
class TestEndToEndScoringRun:
    ...
    def test_three_predictors_one_observation(self, db, adapter):
        _add_bounty(db, pay=1000, paid_predictors=3)
        ...
        # bounty_payments table populated
        pays = db.get_bounty_payments(STREAM, PROVIDER_PK)
        assert len(pays) == 3
    ...
    def test_paid_predictors_cap_honoured(self, db, adapter):
        ...
        _add_bounty(db, pay=1000, paid_predictors=2)
    ...
    def test_leaderboard_aggregates_multiple_observations(self, db, adapter):
        ...
        board = db.get_bounty_leaderboard(STREAM, PROVIDER_PK)
...
class TestJoinedBounties:
    ...
    def test_join_and_lookup_by_stream(self, db):
        ...
        joined = db.get_joined_bounties_for_stream(STREAM, PROVIDER_PK)
        assert len(joined) == 1
        assert joined[0]['host_pubkey'] == HOST_PK

    def test_join_is_idempotent(self, db):
        db.join_bounty(STREAM, PROVIDER_PK, HOST_PK)
        db.join_bounty(STREAM, PROVIDER_PK, HOST_PK)
        assert len(db.get_joined_bounties_for_stream(STREAM, PROVIDER_PK)) == 1

    def test_multiple_hosts_for_same_stream(self, db):
        db.join_bounty(STREAM, PROVIDER_PK, 'host_a')
        db.join_bounty(STREAM, PROVIDER_PK, 'host_b')
        joined = db.get_joined_bounties_for_stream(STREAM, PROVIDER_PK)
        host_pks = {r['host_pubkey'] for r in joined}
        assert host_pks == {'host_a', 'host_b'}

    def test_leave_bounty(self, db):
        db.join_bounty(STREAM, PROVIDER_PK, HOST_PK)
        db.leave_bounty(STREAM, PROVIDER_PK, HOST_PK)
        assert db.get_joined_bounties_for_stream(STREAM, PROVIDER_PK) == []

    def test_stream_not_joined_returns_empty(self, db):
        assert db.get_joined_bounties_for_stream('other', PROVIDER_PK) == []
...
class TestLateArrival:
    ...
    def test_late_prediction_gets_scored_on_second_pass(self, db, adapter):
        ...
        score_and_pay(db, adapter, STREAM, PROVIDER_PK, HOST_PK,
                      seq_num=1, observed_value=100.0)
        assert len(db.get_bounty_payments(STREAM, PROVIDER_PK)) == 1
